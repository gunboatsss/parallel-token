...
contract ParallelToken {
    ...
    function burnMany(uint256[] calldata _id) public returns (uint256 redeemed) {
        ...
        require(length > 0);
        address underlying = idToTokenData[_id[0]].underlyingERC20;
        for (uint256 i; i < length; i++) {
            uint256 currentId = _id[i];
            require(idToTokenData[currentId].underlyingERC20 == underlying);
            require(idToTokenData[currentId].owner == msg.sender);
            redeemed += idToTokenData[currentId].amount;
            delete idToTokenData[currentId];
            emit Burn(currentId, msg.sender, redeemed);
        }
